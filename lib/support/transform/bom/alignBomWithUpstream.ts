...
export function alignBomWithUpstream(springBootVersion: string): SimpleProjectEditor {
  ...
  return async project => {
    ...
    await setPomVersion(project, springBootVersionToBomVersion(springBootVersion));
    await setPomParentVersion(project, springBootVersion);
    await setPomProperty(project, SPRING_BOOT_VERSION_PROPERTY_NAME, springBootVersion);
    ...
    for (const key of Object.keys(pom.project.properties)) {
      if (!IGNORED_PROPERTIES.includes(key) && upstreamProperties.has(key)
        ...
        await setPomProperty(project, key, upstreamProperties.get(key));
...
async function setPomProperty(project: Project, key: string, value: string): Promise<void> {
  logger.info(`Setting pom property ${key}=${value}`);
  await doWithAllMatches(project, new XmldocFileParser(), "pom.xml", `/project/properties/${key}`, match => {
    match.$value = match.$value.replace(new RegExp(`<${key}>[\\s\\S]*?<\/${key}>`), `<${key}>${value}</${key}>`);
  });
}

async function setPomParentVersion(project: Project, version: string): Promise<void> {
  logger.info(`Setting pom parent version to ${version}`);
  await doWithAllMatches(project, new XmldocFileParser(), "pom.xml", "/project/parent/version", match => {
    match.$value = match.$value.replace(new RegExp("<version>[\\s\\S]*?<\/version>"), `<version>${version}</version>`);
  });
}

async function setPomVersion(project: Project, version: string): Promise<void> {
  logger.info(`Setting pom version to ${version}`);
  await doWithAllMatches(project, new XmldocFileParser(), "pom.xml", "/project/version", match => {
    match.$value = match.$value.replace(new RegExp("<version>[\\s\\S]*?<\/version>"), `<version>${version}</version>`);
  });
}
