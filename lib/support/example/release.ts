...
export function getCommunityEditor(springBootVersion: string): AnyProjectEditor {
...
export function getProdEditor(springBootVersion: string, bomVersion: string): AnyProjectEditor {
  return (project: Project) => getCurrentVersionWithoutSnapshot(project)
...
export function getVersionBumpEditor(): AnyProjectEditor {
  return (project: Project) => getCurrentVersion(project)
      .then(version => getNextExampleVersion(version))
      .then(version => setProjectVersion(project, version));
}

async function tag(params: ReleaseParams, branch: string, editor: AnyProjectEditor): Promise<void> {
...
function editInBranch(params: ReleaseParams, commitBranch: string, commitMessage: string, editor: AnyProjectEditor):
    Promise<EditResult> {
  return editOne(
      params.context,
