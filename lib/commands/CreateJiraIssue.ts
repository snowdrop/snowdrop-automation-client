...
export class CreateJiraIssue implements HandleCommand {
    ...
    public async handle(context: HandlerContext): Promise<HandlerResult> {
        logger.debug(`${this.username} is attempting to create a JIRA issue on ${this.host} for project=${this.project} with a summery ='${this.summary}' assigned to ${this.assignee}`);
